...
class ParentServicesController extends Controller
{
 ...
 try {
    ...
    /**
     * Get trending parent services based on recent orders count.
     * Optional filters:
     * - days: lookback window (default 30)
     * - service_id: restrict to a specific service
    ...
    public function trending(Request $request)
    {
        try {
            ...
            $lookbackDate = now()->subDays(max($days, 1));

            $query = ParentServices::query()
                ->where('status', 'active');
            ...
            // Join order_items for recent count
            $query->leftJoin('order_items', function ($join) use ($lookbackDate) {
                $join->on('order_items.parent_service_id', '=', 'parent_services.id')
                    ->where('order_items.created_at', '>=', $lookbackDate);
            });

            $query->select([
                'parent_services.id',
                'parent_services.name',
                'parent_services.slug',
                'parent_services.description',
                'parent_services.image',
                'parent_services.images',
                'parent_services.tags',
                'parent_services.includes',
                'parent_services.excludes',
                'parent_services.price',
                'parent_services.discount',
                'parent_services.discount_type',
                'parent_services.discount_start_date',
                'parent_services.discount_end_date',
                'parent_services.notes',
                'parent_services.status',
                'parent_services.service_id',
                DB::raw('COUNT(order_items.id) AS recent_orders_count'),
            ])
            ->groupBy([
                'parent_services.id',
                'parent_services.name',
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Failed to retrieve trending parent services.',
                'error' => $e->getMessage(),
                'data' => null,
            ], 500);
        }
